...
public class ControllerPart {
    ...
    @PostMapping
    public ResponseEntity<DataPart> partRegistration(@Valid @RequestBody DataPart dataPart){
        Part part = this.service.registration(dataPart);
        return ResponseEntity.created(URI.create("/"+part.getId())).body(new DataPart(part)) ;
    }
    @GetMapping
    ...
    @PatchMapping ("/{id}")
    public ResponseEntity<DataAllPart> updatePart(@PathVariable(name = "id") Long id, @RequestBody DataPart part){
